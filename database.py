...
def init_db():
    """
    Создаёт таблицы лидов, лимитов, блокировок, историй и классификации.
    """
    ...
    # Таблица истории сообщений (памяти)
    cursor.execute("""
    ...
    # Таблица классификации пользователей
    cursor.execute("""
...
def log_security_threat(user_id, threat_type, message_text, platform="telegram"):
    ...
    cursor.execute(
        ...
        (str(user_id), platform, threat_type, message_text[:200])
    )
    conn.commit()
    conn.close()
...
# ========== ДОБАВЛЕННЫЕ ФУНКЦИИ ДЛЯ ПАМЯТИ И КЛАССИФИКАЦИИ ==========

def get_user_classification(user_id, platform="telegram"):
    """Возвращает текущую категорию пользователя"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute(
        "SELECT classification FROM user_classifications WHERE user_id = ?",
        (str(user_id),)
    )
    result = cursor.fetchone()
    conn.close()
...
def classify_user(user_id, classification, platform="telegram"):
    """Сохраняет новую категорию пользователя"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute(
        "INSERT OR REPLACE INTO user_classifications (user_id, platform, classification) VALUES (?, ?, ?)",
        (str(user_id), platform, classification)
    )
    conn.commit()
    conn.close()
...
def mark_user_as_spam(user_id, platform="telegram"):
    """Помечает пользователя спамером"""
...
def save_message_to_history(user_id, user_message, bot_response, platform="telegram"):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO message_history (user_id, platform, user_message, bot_response) VALUES (?, ?, ?, ?)",
        (str(user_id), platform, user_message, bot_response)
    )
    conn.commit()
    conn.close()
...
def get_last_messages(user_id, platform="telegram", limit=10):
    """Возвращает последние сообщения для контекста памяти"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute(
        "SELECT user_message, bot_response FROM message_history WHERE user_id = ? AND platform = ? ORDER BY id DESC LIMIT ?",
